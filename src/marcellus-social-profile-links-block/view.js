...
// console.log( 'Hello World! (from Marcellus social profile links block)' );
...
const viewExpressSocials = () => {
	...
	document
		.querySelectorAll(
			'.marcellus-social-profile-links-item-container:not([data-rendered="true"])'
		)
		.forEach( ( container ) => {
			try {
				...
				console.error( 'Social Profile Links Render Error:', err );
			}
		} );
};
