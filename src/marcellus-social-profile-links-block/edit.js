...
export default function Edit( { attributes, setAttributes } ) {
	...
	useEffect( () => {
		apiFetch( { path: '/wp/v2/settings' } )
			.then( ( response ) => {
				const data = response.marcellus_social_profile_links_data;
				if ( data ) {
	...
	const blockSocialHasData =
		attributes.social &&
		attributes.social.length > 0 &&
		( attributes.social[ 0 ].label || attributes.social[ 0 ].logo );
	const activeSocialList = blockSocialHasData
		? attributes.social
	...
	const socialDataPayload = {
		...
		showHideSocials:
			attributes.showHideSocials !== undefined
				? attributes.showHideSocials
				: globalSettings.showHideSocials !== false,
		showText:
			attributes.showText !== undefined
				? attributes.showText
				: globalSettings.showText !== false,
		hasBorder:
			attributes.hasBorder !== undefined
				? attributes.hasBorder
				: globalSettings.hasBorder !== false,
	...
	const isListEmpty =
		! socialDataPayload.social ||
		socialDataPayload.social.every( ( row ) => ! row.label && ! row.logo );
	...
	return (
		<div { ...blockProps }>
			...
			<InspectorControls>
				...
				>
					<ToggleControl
						...
						onChange={ ( val ) =>
							setAttributes( { showHideSocials: val } )
						}
					/>
					<ToggleControl
						label={ __(
			...
			<div className="marcellus-social-profile-links-block-editor-canvas">
				{ ! socialDataPayload.showHideSocials ? (
					...
					>
						...
						>
							❌{ ' ' }
							{ __(
								'Social block display is toggled hidden.',
								'marcellus-social-profile-links'
							) }
						</p>
					</div>
					...
					>
						{ __(
							'You have not added any social links yet. Add them in the block sidebar or global options.',
							'marcellus-social-profile-links'
						) }
					</p>
				) : (
