...
const SocialsListRenderer = ( { socialData } ) => {
	...
	return (
		<div className="marcellus-social-profile-links-box">
			{ socialData.social.map( ( row, index ) => {
				...
				const itemStyle = {
					flexDirection:
						flexDirections[ socialData.labelPosition ] || 'row',
					...
					boxShadow: itemHasBorder
						? '0 1px 2px rgba(0,0,0,0.05)'
						: 'none',
					border: itemHasBorder
						? `${ socialData.borderWidth } solid ${ socialData.borderColor }`
						: 'none',
				};
				...
				return (
					<a
						...
						className="marcellus-social-profile-links-item"
						style={ itemStyle }
					>
						{ /* isolate icon-wrapper context inside the loop */ }
						<span className="marcellus-social-profile-links-icon-wrapper icon-wrapper">
							{ row.logo ? (
								<img
									src={ row.logo }
									alt=""
									className="marcellus-social-profile-links-img"
								/>
							) : (
								<div className="marcellus-social-profile-links-fallback-icon">
									🔗
								</div>
							) }
						</span>
						{ socialData.showText && (
							<span className="marcellus-social-profile-links-text-node">
								{ row.label ||
									__(
										'Unnamed Link',
			...
			<style>
				{ `
                    .marcellus-social-profile-links-box {
                        display: flex;
                        flex-wrap: wrap;
                    ...
                    .marcellus-social-profile-links-item {
                        display: inline-flex;
                        align-items: center;
                        ...
                        background: ${
							socialData.hasBorder ? '#fff' : 'transparent'
						};
                        box-shadow: ${
							socialData.hasBorder
								? '0 1px 2px rgba(0,0,0,0.05)'
                    ...
                    .marcellus-social-profile-links-icon-wrapper {
                        display: inline-flex;
                        align-items: center;
                        justify-content: center;
                    ...
                    .marcellus-social-profile-links-img {
                        width: ${ socialData.iconSize };
                        height: ${ socialData.iconSize };
                        object-fit: contain;
                    }
                    .marcellus-social-profile-links-fallback-icon {
                        width: ${ socialData.iconSize };
                        height: ${ socialData.iconSize };
                        display: flex;
                        align-items: center;
                    ...
                    .marcellus-social-profile-links-text-node {
                        line-height: 1;
                    }
                ` }
			</style>
