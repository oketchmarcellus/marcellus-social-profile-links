...
const SettingsPage = () => {
	...
	const handleSave = async () => {
		...
		try {
			await apiFetch( {
				...
				method: 'POST',
				data: {
					marcellus_social_profile_links_data: socialData, // Sends the fully bundled state object
				},
			} );
			createSuccessNotice(
				__(
			...
			if ( error.code === 'rest_invalid_param' || error.status === 400 ) {
				generalizedMessage = __(
					'One or more styling selections contain an invalid format.',
					'marcellus-social-profile-links'
				);
	...
	return (
		<div className="wrap">
			...
			<Panel className="marcellus-socials-panel">
				<TabPanel
					className="marcellus-socials-tabs"
							...
							<div style={ { flex: 1, padding: '24px 10px' } }>
								{ tab.name === 'socials' && (
									<Flex
										align="start"
										gap={ 6 }
										className="marcellus-socials-responsive-flex"
									>
										{ /* Left Repeater Fields Panel */ }
										{ /* PASSED DATA PROPS DOWN TO CHILD PANEL */ }
										<FlexItem className="marcellus-socials-form-column">
				...
				<Flex
					justify="flex-end"
					className="marcellus-socials-footer-actions"
				>
					<Button
						variant="primary"
						isBusy={ isSaving }
			...
			<style>
				{ `
                    ...
                    .marcellus-socials-responsive-flex {
                        width: 100%;
                        flex-direction: column;
                    }

                    .marcellus-socials-form-column {
                        flex: 1;
                    }

                    .marcellus-socials-preview-column {
                        flex: 1;
                        top: '20px';
                    }

                    .marcellus-socials-footer-actions {
                        margin-top: '12px';
                    }
