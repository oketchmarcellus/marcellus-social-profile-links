...
const SocialsPanel = ( { socialData, setSocialData } ) => {
	...
	const removeRepeaterRow = ( indexToKill ) => {
		const updatedSocials = socialData.social.filter(
			( _, index ) => index !== indexToKill
		);
		setSocialData( { ...socialData, social: updatedSocials } );
	};
		...
		<div>
			...
			>
				...
				{ socialData.social.map( ( row, index ) => (
					...
					>
						...
						<FlexItem className="marcellus-social-profile-links-logo-column">
							<div
								style={ {
									position: 'relative',
									width: '55px',
							...
							>
								{ row.logo ? (
									<img
										className="marcellus-social-profile-links-logo"
										src={ row.logo }
										onClick={ () =>
			...
			<style>
				{ `
                    ...
                    .marcellus-social-profile-links-logo-column {
                        align-items: center;
                        flex-basis:70px;
                        flex-grow: 0;
                    }

                    .marcellus-social-profile-links-logo {
                        width: 35px;
                        height: 35px;
                    }

                    .marcellus-social-profile-links-field-inputs-column{
                        display: flex;
